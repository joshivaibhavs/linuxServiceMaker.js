...
const getAppNameAndTemplate = (args) => {
  ...
  if (!/^[\w\d]+$/.test(appName))
    throw new Error(
      'Invalid app name. App name can contain letters, numbers and underscores.'
    );
  if (!fs.existsSync(matchDir))
    throw new Error(`The given directory ${matchDir} doesn't exist.`);
  if (!fs.existsSync(`${matchDir}/${entryPoint}`))
    throw new Error(`The given file ${entryPoint} doesn't exist.`);
  if (fs.existsSync(`${matchDir}/${appName}.service`))
    throw new Error(
      `The file ${appName}.service already exists in the directory ${matchDir}. Please rename the file or choose another name for your service.`
    );
  let envVars = null;
  const lsmVarsPath = `${matchDir}/lsm.env`;
  if (fs.existsSync(lsmVarsPath))
    envVars = fs
      .readFileSync(lsmVarsPath)
      .toString()
      .split('\n')
...
const main = () => {
  if (os.platform() !== 'linux') {
    console.log(
      'Non linux system found. The script can only run on linux systems.'
    );
    return;
  }
  try {
    ...
    const { appName, template, matchDir, entryPoint } = getAppNameAndTemplate(
      updatedArgs
    );
